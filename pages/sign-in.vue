...
async function loginAttempt() {
    ...
    try {
        const { data, error } = await useFetch<any>(`${AUTH_SERVICE_URL}/auth/login`, {
            method: "POST",
            body: userInfo,
            headers: {
...
<template>
    <section class="sign-in-container flex items-center container px-10 md:px-20 md:py-10 min-h-screen min-w-full">
        <div class="sign-in-form w-96">
            ...
            <LabelInput v-model="userInfo.password" type="password" placeholder="Your secret password" label="Enter your Password" />
            ...
            <div class="form-submit my-4">
                <button v-if="!authenticating" @click="loginAttempt" class="shadow-xl px-8 py-3 text-sm min-w-full bg-orange-500 text-white rounded-xl">
                    Proceed to Login
                </button>
                <Loading v-else/>
            </div>
